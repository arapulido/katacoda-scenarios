...
Log into [Datadog](https://app.datadoghq.com/) and navigate to the [Kubernetes account set up page](https://app.datadoghq.com/account/settings#kubernetes). Copy the command that reads `kubectl create secret generic datadog-secret --from-literal api-key="<your_api_key>" and paste it in the terminal. This will create a Kubernetes secret to make sure the Datadog agent is able to send data to your Datadog account.
